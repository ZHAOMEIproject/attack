...
contract USDALOCK is Initializable, AccessControlUpgradeable,
UUPSUpgradeable {
    // 正式版需要注释的。
    uint256 fack_time;
    ...
    function getuserorders(address sender)view public returns(
        ...
        {//_userorders
            ...
            for (uint256 i = 0; i < length; i++) {
                uint256 miniendtime=i;
                for (uint256 j = i+1; j < length; j++) {
                    if (userorders._userorders[miniendtime].endtime<userorders._userorders[j].endtime) {
                        miniendtime=j;
                    }
                }
                s_order memory swap = userorders._userorders[miniendtime];
                userorders._userorders[miniendtime]=userorders._userorders[i];
                userorders._userorders[i]=swap;
            }
        }
        {//现在中断可提取利息
            ...
            for (uint256 i = 0; i < length; i++) {
                uint256 miniendtime=i;
                for (uint256 j = i+1; j < length; j++) {
                    if (userorders._interestorders[miniendtime].endtime<userorders._interestorders[j].endtime) {
                        miniendtime=j;
                    }
                }
                s_interestorder memory swap = userorders._interestorders[miniendtime];
                userorders._interestorders[miniendtime]=userorders._interestorders[i];
                userorders._interestorders[i]=swap;
            }
        }
        {
            ...
            // userorders.annual_ngt=(pool.total_reward)*(totalMarkup_share/snapshots[snapshots.length-1].totalamount);
    ...
    function Calculate_benefits(order memory now_order)view public returns(uint256 benefits){
        ...
        snapshot memory now_snapshot = snapshots[l-1];//获取开始锁的快照
        uint256 endtime=now_snapshot.start_time+locktypes[now_order.typeoflock].time;
        // require(false,uintToString(endtime));
        for(;l<snapshots_l;l++){
            if (endtime<snapshots[l].start_time){
                // console.log(endtime,snapshots[l].start_time);
        ...
        console.log("benefits,now_order.interest",benefits,now_order.interest);
        return (benefits-now_order.interest);
    }
    ...
    function req_interestorder()public{
        ...
        for (uint256 i = 0; i < l; i++) {
            ...
            uint256 reward = Calculate_benefits(n_order);
            console.log("orders[sender][i].interest ",orders[sender][i].interest);
