...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function fix_getHeakthFactor(address account) public view returns(uint, uint){
        (,uint256 sumCollateral,uint256 sumBorrowPlusEffects)=getHeakthFactor(account);
        return (sumCollateral,sumBorrowPlusEffects);
    }
    function getHeakthFactor(address account) public view returns(Error, uint, uint){
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // sumBorrowPlusEffects += oraclePrice * borrowBalance
            vars.sumBorrowPlusEffects = mul_ScalarTruncateAddUInt(vars.oraclePrice, vars.borrowBalance, vars.sumBorrowPlusEffects);
            // return (Error.NO_ERROR,vars.oraclePrice.mantissa,vars.cTokenBalance);
        }
    ...
    function getcollateralFactor(CToken cToken)public view returns(uint256){
